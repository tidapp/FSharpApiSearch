...
FSharpApiSearch.Console interactive mode directive:
  ...
  #substring [enable|disable]
      Enables or disables the substring matching of type name.
  ...
  let rec loop (client: Lazy<FSharpApiSearchClient>) arg =
    ...
    | OptionSetting "#substring" SearchOptions.Substring arg.SearchOptions newOpt -> loop client { arg with SearchOptions = newOpt }
